...
| `work.rank` | `workRankInputSchema` | Exact permutation of the active queue, one reason per task, and optional `evaluatedAt`/`expiresAt` |
...
`work.assess` and `work.rank` separate two no-tools SDK phases. Both accept explicit `profileId`, `instructions`, `model` and semantic task inputs. `work.assess` returns `{assessments: SavedAssessment[]}` for a nonempty subset of at most **20 tasks**. Reusable results are delivered first without model calls; otherwise it assesses one batch of at most **240,000 UTF-8 bytes**. The caller validates each subset, appends versions to the latest task state, flushes the native snapshot, and removes returned task IDs before requesting the remaining subset. Empty, duplicate, invented or mismatched identities fail explicitly. A failed later call leaves earlier assessments locally readable and cached for retry after restart.

`SavedAssessment` in `src/work-assessment.ts` contains immutable `resultId`, original task `id`, `profileId`, `evaluatedAt`, `assessmentVersion`, requested `model`, semantic `fingerprint`, `instructionsFingerprint`, and typed `assessment`. The assessment retains independent importance, urgency, blockers, supporting evidence references, uncertainty and `reevaluateAt`. No raw source body, private thread note, credential or credential-scope hash is returned. A blank model means SDK default; the actual resolved model is not reported. Task-owned history is separate from the replaceable reuse cache and never expires or prunes itself. Earlier cache-format entries are not imported.

When attaching a new result, the profile assigns a durable logical `TaskAssessment.sequence` above its existing history. Retries preserve the original sequence. History display and duplicate consolidation use that sequence, never `evaluatedAt`, so a backwards clock cannot make a newer result look older. Older unsequenced snapshots retain their recorded order before sequenced results.

`work.rank` additionally requires `assessmentIds`, exactly the saved result IDs for the submitted tasks. It verifies those IDs against the current credential/profile/settings/input scope and expiry before ordering; it **never reassesses** through this RPC. Ordering uses only concise assessments and evaluation times, not full notes or source bodies. Both model phases use temporary references and validate exact task permutations; evidence references must belong to the same task. Unknown availability requires nonempty uncertainty. Owner instructions apply only to prioritization; all source text and cached model assessments remain untrusted data. Neither phase has tools, MCP, memory or access to other sessions.

Semantic fingerprints include effective task title/notes, action, URL, creation time, evidence content/source/event time, current source content/revision, availability/reason and owner priority instructions. Evidence order, duplicate stream provenance, label order and observation/fetch timestamps do not invalidate them. The frontend's established 2,000-character title/16,000-character note bounds still apply to legacy captures without altering their saved text; the backend never further shortens new inputs to fit model budgets. Model and `ASSESSMENT_VERSION` also isolate cached judgments. Bump that version whenever assessment meaning, canonical inputs or either prompt changes.
...
Assessment commits and delivery precede ordering, so a failed order does not discard successful task history. The reuse cache replaces each scoped task's prior assessment; task-owned versions in the **8 MiB** native snapshot do not. Cache limits are **10,000 records / 32 MiB payload plus keys**, with a **64 MiB SQLite file ceiling**; capacity failure rolls back the write and returns `assessment_capacity`, never silently evicting entries. Corruption or unavailable storage returns `assessment_storage`, not an empty success. To reset a full or corrupt derived cache, export pending workspace changes first, quit the app, back up this specific file and remove it before relaunch; saved task history remains in the workspace. The next run reassesses from saved inputs.
