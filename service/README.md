...
`work.rank` runs up to two separate no-tools SDK passes. The first assesses only new, changed or expired tasks from their full effective input, recording independent importance, urgency, blockers, supporting evidence references, uncertainty and `reevaluateAt`. It never uses the old relative ranking reasons as intrinsic assessments. The second orders **every active task** using only those concise assessments and their evaluation times, not full task notes or source bodies. Both passes use temporary references and validate exact task permutations; evidence references must belong to the same task. Unknown availability requires nonempty uncertainty. Owner instructions apply only to prioritization; all source text and cached model assessments remain untrusted data. Neither pass has tools, MCP, memory or access to other sessions.
...
The cache is backend-owned SQLite at `sdk-sessions/work-assessments.sqlite3` beneath the private app data directory. Directories/files use modes 0700/0600 on creation; SQLite uses synchronous full commits and a bounded rollback journal. It stores derived assessments and one comparative order per scope, not raw task bodies or credentials. A domain-separated SHA-256 scope includes the ephemeral `gh` credential, exact instructions, model and format version. No credential or scope digest reaches renderer results or logs. Both passes pin the same in-memory credential, preventing a mid-run account switch from mixing accounts. Credential rotation conservatively invalidates reuse; a cache hit does not claim fresh authentication or source access verification.

Assessment commits precede ordering, so order/model/storage failure does not discard valid assessments. Updates replace each scoped task's prior assessment; queue removal changes only ordering. Limits are **10,000 records / 32 MiB payload plus keys**, with a **64 MiB SQLite file ceiling**; capacity failure rolls back the write and returns `assessment_capacity`, never silently evicting entries. Corruption or unavailable storage returns `assessment_storage`, not an empty success. To reset a full or corrupt derived cache, quit the app, back up this specific file and remove it before relaunch; the workspace and task evidence remain separate. The next run reassesses from saved inputs.

Every fresh pass retains the 240,000-byte input/60,000-byte answer limits, one correction attempt and 180-second SDK deadline; the complete operation stays within 300 seconds. Oversized changed-task input or whole-queue assessment input fails without partial ordering or heuristic fallback. Validated results retain the `orderedIds`/`reasons` contract, adding `evaluatedAt` and `expiresAt`; the desktop preserves the actual evaluation time on cache reuse. During an in-flight edit it excludes stale results for changed tasks, accepts harmless observation timestamp updates, and retains the existing safeguards for Done, captures and changed settings. Empty queues need no cache or SDK access.
