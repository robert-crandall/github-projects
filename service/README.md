...
`work.rank` runs two no-tools SDK phases. Assessment batches contain only new, changed or expired tasks from their full effective input, recording independent importance, urgency, blockers, supporting evidence references, uncertainty and `reevaluateAt`. Each batch contains at most **20 tasks and 240,000 UTF-8 bytes**, receives its own evaluation time and saves before the next batch starts. A failed later batch leaves earlier assessments available for retry after restart. Assessment never uses the old relative ranking reasons as intrinsic assessments. One ordering pass then orders **every active task** using only those concise assessments and their evaluation times, not full task notes or source bodies. Both phases use temporary references and validate exact task permutations; evidence references must belong to the same task. Unknown availability requires nonempty uncertainty. Owner instructions apply only to prioritization; all source text and cached model assessments remain untrusted data. Neither phase has tools, MCP, memory or access to other sessions.
