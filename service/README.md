...
| `work.assess` | `workRankInputSchema` without `assessmentIds`/`assessments`, requiring `profileId` | A nonempty subset of at most 20 immutable assessments |
| `work.rank` | `workRankInputSchema`, requiring `profileId`, `assessmentIds` and `{taskId, result}` assessment snapshots | Exact permutation of the active queue, one reason per task, and optional `evaluatedAt`/`expiresAt` |
...
`work.rank` requires both saved `assessmentIds` and typed `{taskId, result}` snapshots read from native history. The native reader enforces task/profile ownership, including consolidated aliases; the sidecar validates the profile and exact task/result permutations again. Ranking does not depend on the disposable assessment cache, input-fingerprint equality or an old reevaluation time. It **never reassesses**. The renderer prefers a saved version matching the assessor configuration, falling back to the latest judgment with a named warning when settings or format differ. Missing history returns `assessment_required`. `force: true` bypasses only order reuse. Ranking transport omits raw notes, evidence and source bodies; `assessmentInputFingerprint` carries their current digest for changed-input warnings and cache invalidation, not as an authentication proof. Ordering receives concise historical assessments, titles, evaluation times and current actionability/PR state. Saved judgments remain untrusted data, not authority to execute instructions. Neither model phase has tools, MCP, memory or access to other sessions.

Before ordering, the existing `work.collect` observe-only path refreshes tracked GitHub issues/PRs in batches of at most 100 URLs, without collecting candidates or invoking a model. `stateOnly: true` (valid only with `observeOnly: true`) omits stable source context from responses so long descriptions cannot exhaust the 1 MiB transport budget. Stream/model fields remain required but are ignored in observe-only mode. Repository, commit, release and discussion links stay rankable without entering this issue/PR observation path. Pipeline collection already observes tracked sources; the readiness pass only reads remaining URLs. Serialized, generation-fenced replies replace readiness even after a backwards local clock correction.

Source observations carry optional structured `pullRequest` state: observation time, head, draft flag, check summary, coverage and readiness, with author/merge/review state when known. These facts no longer appear inside the stable source body/revision. A failed per-source observation clears prior readiness to unknown, while whole-operation failures stop ordering. The default ordering prompt demotes non-ready review actions without demoting work to fix the owner's own PR.
...
Assessment payloads retain their existing one-minute-to-24-hour `reevaluateAt` field for format compatibility and assessment-call retry caching. It does not expire native history or gate prioritization. The desktop automatically assesses only tasks without saved history; explicit selected/task reassessment bypasses assessment reuse. Comparative ordering keeps its own one-minute-to-**one-hour** lifetime, independent of judgment age. Its cache key includes saved judgments, current task inputs, structured PR state and prioritizer configuration; an observation timestamp alone does not invalidate it. Expired ordering only reruns the ordering pass. Future-dated cached orders and orders that expire before completion fail or refresh explicitly.

The cache is backend-owned SQLite at `sdk-sessions/work-assessments.sqlite3` beneath the private app data directory. Directories/files use modes 0700/0600 on creation; SQLite uses synchronous full commits and a bounded rollback journal. It stores derived assessments and one comparative order per scope, not raw task bodies or credentials. A domain-separated SHA-256 scope includes the ephemeral `gh` credential, profile ID, assessor identity and format version. No credential or scope digest reaches renderer results or logs. Each RPC pins its in-memory credential. Credential rotation invalidates derived cache reuse, not owner-held permanent judgments supplied for ranking. Explicit observation checks current source access; a cache hit is never evidence of that access.

Assessment commits and delivery precede ordering, so a failed order does not discard successful task history. The reuse cache replaces each scoped task's prior assessment; permanent native `task_assessments` rows do not. History is outside the **8 MiB** snapshot limit. Cache limits remain **10,000 records / 32 MiB payload plus keys**, with a **64 MiB SQLite file ceiling**; capacity failure rolls back the write and returns `assessment_capacity`, never silently evicting entries. Corruption or unavailable storage returns `assessment_storage`, not an empty success. To reset a full or corrupt derived cache, export pending results first, quit the app, back up this specific file and remove it before relaunch; permanent history remains in workspace.sqlite3 and can be prioritized without reassessment.
