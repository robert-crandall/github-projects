import { z } from 'zod';
import { getRow, transition } from '../domain/engine.ts';
import { emptyWorkspace, reminderSchedules, restoreDesktop } from '../domain/live.ts';
import { stateSchema, type AppState, type Command } from '../types.ts';
import { nativePlatform, snapshotSchema, type NativeTick, type ReminderStatus } from '../platform/native.ts';
...
export type DesktopStatus = {
  ...
  persistence: PersistenceStatus; feedback: string; reminders: ReminderStatus | null; reminderError: string;
};
...
export class DesktopWorkspace {
  ...
  private status: DesktopStatus = {
    ...
    persistence: { saving: false, pending: false, error: '' }, reminders: null, reminderError: '',
  };
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      const envelope = saved.snapshot ? desktopEnvelopeSchema.parse(saved.snapshot.workspace) : null;
      ...
      this.queue = new PersistenceQueue(saved.revision, async (revision, value) => {
        const snapshot = snapshotSchema.parse(JSON.parse(JSON.stringify({
          formatVersion: 1, workspace: value, reminders: reminderSchedules(value.state),
        })));
        const result = await this.platform.workspaceSave(revision, snapshot);
        this.revision = result.revision;
        return result;
      }, persistence => this.publish({ persistence }));
      this.publish({ workspace, loading: false, loadError: '', reminderError: clock.error?.message ?? '' });
  ...
  update(transform: (current: AppState) => AppState): void {
    ...
    const next = { ...workspace, state: transform(workspace.state) };
  ...
  dispatch = (command: Command, feedback = ''): boolean => {
    try {
      this.update(current => {
        ...
        if ('key' in command && command.type !== 'select' && command.key === current.selectedKey) {
          const retained = getRow(next, command.key)?.action;
          if (retained) next = transition(next, { type: 'select', key: `a:${retained.id}` });
        }
        return next;
      });
  ...
  clock(tick: NativeTick): void {
    this.publish({ reminders: tick.reminders, reminderError: tick.error?.message ?? tick.clock.error?.message ?? '' });
    const workspace = this.status.workspace;
    if (!workspace) return;
    try {
      const state = transition(workspace.state, { type: 'clock', now: tick.clock.now });
      const changed = JSON.stringify({ ...state, clock: workspace.state.clock }) !== JSON.stringify(workspace.state);
      const next = { ...workspace, state };
      this.publish({ workspace: next });
      // Native saves emit a tick themselves. A clock-only tick must not start a save loop.
      if (changed) this.queue?.enqueue(next);
  ...
  pendingJson(): string {
    ...
    return JSON.stringify({ formatVersion: 1, workspace: this.status.workspace, reminders: reminderSchedules(this.state) }, null, 2);
  }
  async savedJson(): Promise<string> {
    const saved = await this.platform.workspaceRead();
    return this.platform.exportJson(saved.revision);
  }
  async readReminderStatus(): Promise<void> {
    try { this.publish({ reminders: await this.platform.reminderStatus(), reminderError: '' }); }
    catch (error) { this.publish({ reminderError: message(error) }); }
  }
  async enableReminders(): Promise<void> {
    await this.platform.requestReminderPermission();
    await this.readReminderStatus();
  }
  async retryReminder(key: string): Promise<void> {
    if (!this.status.reminders) throw new Error('Read reminder status before retrying.');
    await this.platform.retryReminder(key, this.status.reminders.revision);
    await this.readReminderStatus();
  }
  async recoverBackup(id: string): Promise<void> {
    ...
    if (backup.snapshot) {
      const envelope = desktopEnvelopeSchema.parse(backup.snapshot.workspace);
