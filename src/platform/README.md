...
`work.reviewCode` has strict service/native input DTOs and browser-safe result validation, but no renderer callers, configured code roles or run storage yet. Its 180-second operation budget includes correction; JSONL/native guards allow 210/240 seconds for cleanup. Code cancellation ACKs keep the target alive until its actual terminal response, unlike legacy write cancellation. `ServiceCallError` preserves service/native error codes for callers. No new transport, global permissions, arbitrary endpoints or GitHub writes are introduced.
