...
`work.collect`, `work.observe`, `work.assess`, `work.rank`, `work.connections`, `work.intake` and `work.ackIntake` are explicit host operations. Collection applies evidence to the latest local state. Observation refreshes tracked GitHub status without collecting new tasks or calling a model. Automatic assessment only handles tasks with no history; explicit selected/task reassessment creates new versions. Each subset commits through native `assessment_append` before ordering. `work.rank` receives validated saved result snapshots and IDs, plus current state; neither age nor a missing derived cache requires reassessment. Intake acknowledgement follows workspace persistence. A failed history append retains a separate pending batch with visible retry/export controls; it does not block task saves. Task Done never uses a GitHub notification write.
