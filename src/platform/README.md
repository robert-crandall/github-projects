...
Requests/replies are bounded to 1 MiB frames, four ordinary concurrent requests, a 150-second native timeout (330 seconds for `work.collect` and `work.rank`), and continuously drained bounded stderr. The work timeout leaves cleanup time after the service's 300-second deadline. IDs correlate out-of-order replies; malformed or unknown responses stop the group. Quit, timeout, acknowledged active cancellation, and service-level cancelled/deadline results TERM then KILL the entire owned process group, including descendants. A no-op cancellation does not interrupt peers. Interrupted GitHub writes remain unconfirmed because they may have reached GitHub; work collection/ranking failures are read-only. The next request lazily starts a clean group.

`github.waiting` accepts only an empty input and reads the fixed **Waiting on me** searches. `ServiceWorkspace.generateWaiting()` is reachable only from the explicit digest command, never notification Refresh or lifecycle events. It coalesces overlapping clicks and validates the complete result before replacing its in-memory snapshot. Failed or malformed results keep the previous report and a visible error. Generation does not update the workspace, persist operation intents, load conversations, or call Copilot. The digest's checkmarks stay in renderer memory; source links use the existing validated `launchGitHub` boundary.

**Add checked items to Tasks** dispatches one local `capture-tasks` command for up to 300 checked source summaries. It uses the same task creation helper as Capture, without overwriting the capture draft or selected reader. Titles and action/source-URL notes enter the existing Task records, not new source metadata or a service request. The selection clears after the workspace accepts the batch. Failed persistence keeps pending Tasks; retry saves them without adding duplicates.
