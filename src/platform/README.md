...
`work.collect`, `work.assess`, `work.rank`, `work.connections`, `work.intake` and `work.ackIntake` are explicit host operations. Collection applies evidence to the latest local state. Observe-only collection with `stateOnly: true` refreshes tracked GitHub issue/PR status without source bodies, new tasks or model calls. Serialized observations are generation-fenced, not rejected on local clock rollback. Automatic assessment only handles tasks with no history; explicit selected/task reassessment creates new versions. Each subset commits through native `assessment_append` before ordering. `work.rank` receives validated saved result snapshots and IDs, plus current state and compact task fingerprints; neither age nor a missing derived cache requires reassessment. Intake acknowledgement follows workspace persistence. A failed history append retains a separate pending batch with visible retry/export controls; it does not block task saves. Task Done never uses a GitHub notification write.
...
Use the controller's `busy` status for dispatch controls, not the visible `active` task. After restore, `active` can be empty while the previous request still owns the shared Copilot slot. The slot releases only after that target settles, before independent terminal persistence finishes; a slow or failed result save does not block new SDK work.
