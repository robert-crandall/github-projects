...
`thread.terminal` is a local checkpoint, not `thread.archive`. It retains the terminal reason and the existing archive-style activity boundary. Confirmed terminal refreshes advance that boundary without acknowledgement or handled IDs. A confirmed open source needs genuinely newer activity to clear it; failed/omitted/unknown state temporarily stops suppression but retains checkpoint provenance. A newer confirmed open result can therefore distinguish terminal-era history from new work. Explicit Restore of a nonterminal source clears its old local terminal hold; user rules still apply. Unknown state never clears manual Archive.
