...
type Destination = { row: Row; kind: 'github' | 'copilot' | 'notification'; action?: 'done' | 'unsubscribe' };
...
function LaterDialog({ row, dispatch, state, close, error }: { row: Row; dispatch: Dispatch; state: AppState; close: () => void; error: string }) {
  ...
  return <Modal title="Keep it for later" close={close}>
    ...
    <form onSubmit={event => {
      ...
      <label>Remind me <span className="muted">optional</span><input type="datetime-local" value={until} onChange={event => setUntil(event.target.value)} /><span className="field-help">{state.timeZone}. Demo clock: {stamp(state.clock, state.timeZone, true)}. No date means no reminder.</span></label>
...
function Detail({ row, state, dispatch, open, later, back, unsaved }: {
  row: Row; state: AppState; dispatch: Dispatch; open: (destination: Destination) => void; later: () => void; back: () => void; unsaved: boolean;
}) {
  ...
  return <article className="detail" aria-label="Selected item">
    ...
    <div className="detail-body">
      ...
      {action?.interpretation === 'pending' && <section className="interpret-panel"><p>Your original text is saved. Structure it with the prototype's sample rules.</p><button className="secondary" onClick={() => dispatch({ type: 'interpret', key: row.key }, 'Sample interpretation applied.')}><Sparkles size={15} />Try simulated interpretation</button></section>}
      {action?.interpretationMessage && <p className={action.interpretation === 'error' ? 'inline-error' : 'notice-inline'}>{action.interpretationMessage}{action.interpretation === 'error' && <button className="text-button" onClick={() => dispatch({ type: 'interpret', key: row.key })}>Retry interpretation</button>}</p>}
      ...
      {row.thread && <section className="thread-controls"><h3>The GitHub notification</h3><p className="muted">{row.thread.notification === 'done' ? 'Acknowledged' : row.thread.notification === 'read' ? 'Read' : 'Unread'} on simulated GitHub · {row.thread.subscribed ? 'Following this conversation' : 'Unsubscribed'}</p>
      ...
      <details className="sources"><summary>Original captures &amp; source history<ChevronDown size={14} /></summary>
        ...
        {!action?.captures.length && !row.thread && <p className="muted">A local sample action. No GitHub source.</p>}
      </details>
...
export function App() {
  ...
  function rowList(items: Row[]) {
    return <ul className="work-list">{items.map(row => <WorkRow key={row.key} row={row}
      ...
      select={() => dispatch({ type: 'select', key: row.key })} open={setDestination} />)}</ul>;
  }
  ...
  return <div className={`app ${selected ? 'has-selection' : ''}`}>
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom">
        ...
        <div className="prototype-label"><span className="demo-dot" />Browser prototype<span>Synthetic data · local only</span></div>
      </div>
    </aside>
    <main id="workspace" className="workspace">
      <header className="workspace-heading"><div><span className="page-eyebrow">YOUR WORKSPACE</span><h1>{viewLabels[state.view]}</h1><p>{viewDescriptions[state.view]}</p></div>
        <div className="refresh-area"><div className="button-row"><button className="quiet triage-trigger" onClick={() => setTriage(true)}><Sparkles size={15} />Triage with Copilot</button><button className="secondary refresh-button" onClick={() => dispatch({ type: 'refresh' })}><RefreshCw size={15} />Refresh</button></div><span>{state.refresh.lastSuccessAt ? `Updated ${stamp(state.refresh.lastSuccessAt, state.timeZone)}` : 'Sample snapshot · not yet refreshed'}</span></div>
      </header>
      ...
      <div className="workspace-grid">
        <section className="queue" aria-label={viewLabels[state.view]}>
          <div className="queue-heading"><h2>{state.view === 'attention' ? 'Requests & commitments' : viewLabels[state.view]}<span>{visible.length}</span></h2>
            <button className="icon-button" aria-label="Reconsider order" title="Reconsider order" onClick={() => dispatch({ type: 'reconsider' }, 'Order reconsidered. Your current action is unchanged.')}><ListFilter size={16} /></button>
          </div>
        ...
        {selected ? <Detail row={selected} state={state} dispatch={dispatch} open={setDestination} later={() => setLater(selected)} back={() => dispatch({ type: 'select', key: null })} unsaved={!!workspace.storageError} />
      ...
      <footer className="workspace-footer"><span><Circle size={11} />Demo clock: {stamp(state.clock, state.timeZone, true)} · {state.timeZone}</span><span>{workspace.storageError ? <><AlertCircle size={12} />Pending changes not saved</> : <><Check size={12} />Saved in this browser</>}</span></footer>
    </main>
