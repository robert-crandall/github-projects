...
Assessment save failures retain visible pending results with separate retry/export controls; notes, Done and capture can still save. Retry or export pending results before quitting. Database backups, raw preservation and restoration include all history. **Backups & recovery** also exports/imports complete JSON copies up to **64 MiB**, with explicit replacement confirmation; larger histories use database backups/raw preservation. Restoring an older copy replaces its task state and history together, preserving the pre-restore database first. Embedded history migrates transactionally after an immutable original backup. Old replaceable cache entries are not imported as history.
