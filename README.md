...
Assessment history belongs to the task, including manual tasks, and travels with its profile in snapshots, backups and exports. Each subset saves before the next assessment or ordering call; retries do not add duplicate versions. The existing **8 MiB** workspace limit still applies, without pruning old results. Save failures retain locally visible pending results with retry and export controls. Results from the old replaceable cache are not imported as history.
