...
test('prototype omits the retired digest and retains ordinary capture', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByRole('heading', { name: 'Inbox', exact: true })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Waiting on me', exact: true })).toHaveCount(0);
  await capture(page, 'Keep my prototype task');
  await page.reload();
  await expect(page.getByRole('heading', { name: 'Tasks', exact: true })).toBeVisible();
  expect((await saved(page)).tasks.at(-1)?.title).toBe('Keep my prototype task');
});
