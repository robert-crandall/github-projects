...
**Run assessor**, **Run now** and scheduled pipelines only assess eligible tasks with no saved history. Any saved version counts, including older formats and changed inputs. Explicit **Assess task** and **Assess selected** create new versions. Progress counts only requested assessment work, so cancellation and retry retain saved batches. **Run prioritizer** refreshes tracked GitHub state, then orders the complete eligible list using permanent judgments and current state without reassessment. Missing judgments or failed refreshes retain the previous order. Filters never narrow whole-list operations. Separate runs do not advance collection coverage or scheduling timestamps.

Store current PR draft, CI, head and readiness observations separately from assessments and stable source content. Current observations override historical readiness claims. Unknown or incomplete checks never become passing checks or a permanent blocker. Demote draft/failing-CI review requests, not actions to fix or finish the owner's own PR. Refresh state through bounded read-only requests on explicit prioritization and scheduled/Run now pipelines, never on task selection or focus. Assessment age never gates ranking; meaningful input changes can flag a saved judgment for explicit reassessment.

To do supports explicit checkboxes and select/clear visible tasks. Selection is local to the profile, list and filter context; context changes, completion and removal invalidate it. Selection never dispatches network work. **Assess selected** and task-detail **Assess task** reuse the durable assessment pipeline with forced reassessment and no collection or ordering. Whole-list prioritization stays separate.

Selected code actions name their eligible issue/PR counts and disclose exclusions. Freeze IDs, profile, workspace generation, source kind/reference and agent policy, revalidating before each serial dispatch. Reserve the existing shared Copilot slot for the whole batch. Saved task failures may continue; cancellation, interruption, lost scope and persistence failures stop remaining jobs. Started batches survive tabs, source filters and Settings navigation with their original scope; only future checkbox selection clears. Profile, workspace or semantic policy changes still stop remaining jobs and cancel the current one. Keep the batch scope, progress and explicit Stop accessible in task views and Settings. Stop never declares idle on cancellation acknowledgement alone. Keep per-task outcomes and unsaved retry/export visible without erasing prior results. Persist actual runs, not an automatically replayable queue.

Assessments include impact, visibility and effort, each high/medium/low/unknown with a short rationale, alongside importance, urgency, blockers, evidence and uncertainty. Missing evidence remains unknown, never an invented effort estimate. Prioritizer edits invalidate order reuse. Assessor edits affect new judgments; prior versions remain usable until explicitly reassessed. Earlier v2 history remains readable without fabricated ratings.
