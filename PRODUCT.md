...
**Run assessor** only assesses eligible tasks with no saved assessment history, without collecting or changing order. Any saved version counts, including outdated, expired and older-format assessments. Explicit **Assess task** and **Assess selected** still create fresh versions. Progress counts only unassessed tasks, so rerunning after cancellation skips saved batches. **Run prioritizer** explicitly recomputes the complete eligible list using current saved assessments and its own instructions, without reassessment. Missing or stale prerequisites explain how to create or refresh assessments and retain the previous order. Filters never narrow either whole-list operation. Separate runs do not advance collection coverage or scheduling timestamps; Run now and scheduled pipelines keep existing reuse policies.
